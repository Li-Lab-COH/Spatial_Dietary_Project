...
lapply(slides, function(slide) {
  ...
  sample.plot.list <- lapply(sample.df$Sample, function(sample_name) {
    ...
    spatial_object <- readRDS(file = paste("./BANKSY_Normalized/Seurat_Objects/",
    ...
    ggsave(path = "./Visium_Slide_QC_Plots/",
...
plot.path.list <-list.files(path = "./Visium_Slide_QC_Plots/",pattern = ".png",full.names = TRUE)




#Plot into PDF
pdf(file = "./Visium_Slide_QC_Plots/All_Slides_QC_UMI_Gene_plots.pdf", 
...
bansky.obj <- readRDS("./5_RT_BANKSY.rds")
obj[['percent.mt']] <- PercentageFeatureSet(obj, pattern = '^mt-')
VlnPlot(obj,features = "percent.mt", pt.size = 0)

spotID.df <- filter(obj@meta.data, percent.mt < 10 &
         nCount_Spatial.008um > quantile(obj$nCount_Spatial.008um, 0.05) &
         nCount_Spatial.008um < quantile(obj$nCount_Spatial.008um, 0.98)) %>%
  rownames_to_column(var = "SpotID")

bansky.obj$quality <- ifelse(rownames(bansky.obj@meta.data) %in% spotID.df$SpotID, 
                             "good", "low") 
DimPlot(bansky.obj,features = "quality")

percent.mt < 10 &
  nCount_Spatial.008um > quantile(sample.obj$nCount_Spatial.008um, 0.05) &
  nCount_Spatial.008um < quantile(sample.obj$nCount_Spatial.008um, 0.98)
