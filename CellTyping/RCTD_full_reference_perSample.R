...
prostate_ST <- subset(prostate_ST, subset = nCount_Spatial.008um >= 90)
...
#############################
log_block("Starting sketch processing...")
DefaultAssay(prostate_ST) <- "Spatial.008um"
...
prostate_ST <- RunUMAP(prostate_ST, reduction = "pca.prostate.sketch",
                       ...
                       dims = 1:15,
                       verbose = TRUE)
log_block("Sketch processing complete. Current memory usage: ", pretty_mem(pryr::mem_used()))
...
RCTD <- create.RCTD(query, reference, max_cores = 8, UMI_min = 100,
